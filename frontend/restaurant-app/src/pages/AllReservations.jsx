...
export default function AllReservations() {
        ...
        setReservations(prevReservations => 
          prevReservations.map(res => 
            res.id === reservationId 
              ? { ...res, checked_in: true, checked_in_at: new Date().toISOString() }
              : res
          )
        );
  ...
  const handleUndoCheckIn = async (reservationId) => {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:8001/api/restaurant/reservations/${reservationId}/undo-check-in`,
        {
          method: 'PATCH',
          headers: {
            'Authorization': `Bearer ${token}`,
      ...
      if (response.ok) {
        ...
        setError(responseData.message || responseData.detail || 'Failed to undo check-in');
        setTimeout(() => setError(''), 4000);
      }
    } catch (err) {
                          ...
                          <>
                            {!isCheckedIn ? (
                              ...
                              >
                                ...
                                {undoingCheckInId === reservationId ? 'Undoing...' : 'Undo Check-in'}
                              </button>
                            )}
                          </>
                        )}
