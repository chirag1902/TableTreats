...
export default function CreateDealPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const dealPayload = {
        ...
        discount_value: formData.discount_type === 'bogo' ? null : parseInt(formData.discount_value),
  ...
  const toggleDealStatus = async (dealId, currentStatus) => {
    try {
      ...
      if (response.ok) {
        setSuccess(`Deal ${!currentStatus ? 'activated' : 'deactivated'} successfully!`);
        await fetchDeals();
        setTimeout(() => setSuccess(''), 3000);
      } else if (response.status === 401) {
        window.location.href = '/signin';
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">{deals.filter(d => d.is_active).length} active deals</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-4">
                  {[
                    ...
                    { value: 'flat_amount', label: 'Flat Amount ($)', icon: DollarSign },
                    { value: 'bogo', label: 'Buy 1 Get 1', icon: Gift }
                  ].map(option => (
                    <button
                      key={option.value}
                      type="button"
                      onClick={() => setFormData(prev => ({ ...prev, discount_type: option.value }))}
                    ...
                    >
                      <option.icon className={`w-6 h-6 ${
                        formData.discount_type === option.value ? 'text-pink-600' : 'text-gray-600'
                      }`} />
                      <span className={`text-sm font-semibold text-center ${
                        formData.discount_type === option.value ? 'text-pink-600' : 'text-gray-700'
                      }`}>
                        {option.label}
                      </span>
                    </button>
        ...
        {deals.filter(d => d.is_active).length > 0 ? (
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex flex-wrap gap-3">
                        ...
                        <div className="flex items-center gap-2 text-sm text-gray-600">
                          ...
                          <span>{deal.time_start} - {deal.time_end}</span>
                        </div>
                      </div>

                      <div className="flex flex-wrap gap-2 mt-3">
                        {deal.valid_days && deal.valid_days.map(day => (
                          <span key={day} className="px-3 py-1 bg-gray-100 text-gray-700 text-xs font-semibold rounded-full">
                            {daysOfWeek[day] ? daysOfWeek[day].slice(0, 3) : day}
                          </span>
                        ))}
                      </div>
                    </div>
                  ...
                  <div className="flex flex-col items-end gap-3">
                    <div className="text-center px-4 py-2 bg-gradient-to-r from-pink-100 to-purple-100 rounded-xl">
                      <p className="text-2xl font-bold text-gray-900">{getDiscountDisplay(deal)}</p>
          ...
          <div className="bg-white rounded-2xl shadow-lg p-12 text-center">
            ...
            <h3 className="text-xl font-semibold text-gray-700 mb-2">No active deals yet</h3>
            <p className="text-gray-500 mb-6">Create your first deal to attract more customers</p>
            <button
