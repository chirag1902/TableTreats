...
import {
  ...
  DollarSign, Percent, Gift, X, Download, Send, Edit2
} from 'lucide-react';
...
export default function BillManagement() {
  ...
  const fetchData = async () => {
    try {
      ...
      const [dealsRes, reservationsRes] = await Promise.all([
        ...
        fetch('http://localhost:8001/api/restaurant/reservations/today/check-in-status', {
          headers
        })
      ]);
      ...
      if (reservationsRes.ok) {
        ...
        setCheckedInReservations(checkedIn.map(res => ({
        ...
        console.error('Reservations fetch failed:', reservationsRes.status, errData);
      ...
      setError(`Connection error: ${err.message}. Ensure backend is running on http://localhost:8001`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchExistingBill = async (reservationId) => {
    try {
      ...
      const response = await fetch(
        `http://localhost:8001/api/restaurant/bills/${reservationId}`,
        {
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json'
  ...
  const handleReservationChange = (reservationId) => {
    const reservation = checkedInReservations.find(r => r.reservation_id === reservationId);
  ...
  const handleCreateBill = async () => {
    ...
    try {
      ...
      if (response.ok) {
        setSuccess(`Bill ${isEditingExistingBill ? 'updated' : 'created'} successfully! Total: $${responseData.total.toFixed(2)}`);
        ...
        if (responseData.detail && responseData.detail.includes('already exists')) {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">Create bills for checked-in customers</p>
            </div>
          </div>
        </div>
      </header>
        ...
        {billExists && existingBill && (
          <div className="mb-6 p-4 bg-amber-50 border border-amber-200 rounded-lg flex items-start gap-3">
            ...
            <div className="flex-1">
              <p className="text-amber-700 font-semibold">Bill Already Created</p>
              <p className="text-amber-600 text-sm">A bill has already been created for this reservation. You cannot create a new bill.</p>
              <div className="mt-2 text-sm text-amber-600">
                <p><span className="font-semibold">Total Amount:</span> ${existingBill.total?.toFixed(2) || '0.00'}</p>
              </div>
            </div>
          </div>
        )}
        ...
        <div className="grid lg:grid-cols-3 gap-8">
            ...
            <div className="bg-white rounded-2xl shadow-lg p-6">
              ...
              {loading ? (
                <div className="text-center py-8">
                  ...
                  <p className="text-gray-500 mt-3">Loading checked-in customers...</p>
                </div>
              ) : checkedInReservations.length === 0 ? (
                <div className="text-center py-8">
                  <AlertCircle className="w-12 h-12 text-gray-300 mx-auto mb-3" />
                  <p className="text-gray-500">No checked-in customers available</p>
                  <p className="text-sm text-gray-400 mt-1">Customers must be checked in before creating a bill</p>
                </div>
              ) : (
                <select
              ...
              {selectedReservation && (
                <div className="mt-4 p-4 bg-purple-50 rounded-lg">
                  <p className="text-sm text-gray-700"><span className="font-semibold">Name:</span> {selectedReservation.customer_name}</p>
                  <p className="text-sm text-gray-700"><span className="font-semibold">Email:</span> {selectedReservation.customer_email}</p>
                  <p className="text-sm text-gray-700"><span className="font-semibold">Phone:</span> {selectedReservation.customer_phone}</p>
                  <p className="text-sm text-gray-700"><span className="font-semibold">Guests:</span> {selectedReservation.number_of_guests}</p>
                </div>
              )}
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-lg p-6">
              ...
              {billExists ? (
                <div className="text-center py-8">
                  ...
                  <p className="text-gray-600 font-semibold">Bill Already Created</p>
                  <p className="text-sm text-gray-500 mt-2">You cannot add items because a bill has already been created for this customer.</p>
                </div>
              ) : (
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => setManualItemForm({...manualItemForm, unit_price: e.target.value})}
                        placeholder="0.00"
                        step="0.01"
                        min="0"
                        className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-pink-500 outline-none"
          ...
          <div className="space-y-6">
                  ...
                  billItems.map((item) => {
                    ...
                    return (
                      <div key={item.dish_id} className="border border-gray-200 rounded-lg p-3">
                        ...
                        <select
                          value={item.promo_id || ''}
                          onChange={(e) => applyDealToItem(item.dish_id, e.target.value)}
                        ...
                        >
                          ...
                          {deals.filter(d => d.is_active).map(deal => (
                            <option key={deal.id} value={deal.id}>
                              {deal.title} ({deal.discount_type === 'percentage' ? `${deal.discount_value}%` : deal.discount_type === 'bogo' ? 'BOGO' : `$${deal.discount_value}`})
                            </option>
                          ))}
                        </select>
                      </div>
            ...
            <button
              ...
              disabled={loading || billItems.length === 0 || !selectedReservation || billExists}
            ...
            >
              ...
              {billExists ? 'Bill Already Created' : loading ? 'Processing...' : 'Create Bill'}
            </button>
          </div>
        </div>
      </main>
